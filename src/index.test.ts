...
describe('all files', () => {
  ...
  test.concurrent.each([...Array(fs.readdirSync(path.join('test')).length-1)].map((_, index) => index + 1))('File %i', async (i) => {
    ...
    const stringifyOutput = Math.random() < 0.5;

    //---JS
    // eslint-disable-next-line no-console
    console.table({'withStringInput': withStringInput, 'withSource': withSource, 'src': path.join('test', `test${i}.csv`), 'withDest': withDest, 'dest': path.join('res', `res${i}.csv`), 'sortColumn': col, 'reverse': reverse, 'sortWithHeader': sortWithHeader, 'withCallback': withCallback, 'stringifyOutput': stringifyOutput});
    ...
    if(stringifyOutput){
      testStrigification(res, withStringInput, i, withDest, col, reverse, sortWithHeader);
    }
...
async function testStrigification(res: any, withStringInput: boolean, i: number, withDest: boolean, col: number, reverse: boolean, sortWithHeader: boolean) {
  let srtingifiedRes = '';

  if(!withStringInput) srtingifiedRes = await sort({stringifyOutput: true, src: path.join('test', `test${i}.csv`), dest: withDest? path.join('res', `res${i}.csv`):null, sortColumn: col, reverse: reverse, sortWithHeader: sortWithHeader}) as string;
  if(withStringInput) srtingifiedRes = await sort({stringifyOutput: true, csvString: fs.readFileSync(path.join('test', `test${i}.csv`)).toString(), dest: withDest? path.join('res', `res${i}.csv`):null, sortColumn: col, reverse: reverse, sortWithHeader: sortWithHeader}) as string;

  const firstLine = srtingifiedRes.split('\n')[0];
  const delimiter = String.fromCharCode(recognizeDelimiter(Buffer.from(srtingifiedRes, 'utf-8')));
  ...
  import('csv-parse/sync').then(module => {
    const parsedRes = module.parse(srtingifiedRes, { //same parse algorithm as in module
      bom: true,
      columns: columns,
      delimiter: delimiter,
      ltrim: true,
    ...
    expect(res).toStrictEqual(parsedRes);
  });
}